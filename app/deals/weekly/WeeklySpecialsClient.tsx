...
export default function WeeklySpecialsClient() {
  const [searchQuery, setSearchQuery] = useState('')
  const [selectedCategory, setSelectedCategory] = useState('All Categories')
  const [selectedNeighborhood, setSelectedNeighborhood] = useState('All Locations')
  const [selectedDay, setSelectedDay] = useState(getCurrentDay())
  const [activeOnly, setActiveOnly] = useState(true) // Default to active only for weekly specials
  ...
  // Group deals by day for calendar view
  const dealsByDay = useMemo(() => {
    const grouped: Record<string, Deal[]> = {}
    filteredDeals.forEach(deal => {
      if (deal.daysOfWeek && deal.daysOfWeek.length > 0) {
        deal.daysOfWeek.forEach(day => {
          if (!grouped[day]) grouped[day] = []
          grouped[day].push(deal)
        })
      }
    })
    return grouped
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-600">
                {filteredDeals.length} {filteredDeals.length === 1 ? 'special' : 'specials'} found
              </span>
              <select
                value={sortBy}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filteredDeals.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm p-12 text-center">
              ...
              <h3 className="text-xl font-bold text-pittsburgh-black mb-2">No weekly specials found</h3>
              <p className="text-gray-600 mb-6">
                Try adjusting your filters or check back later for new weekly specials.
              </p>
              <button
            ...
            <>
              ...
              {viewMode === 'grid' && (
                <div className="mb-8">
                  <h2 className="text-2xl font-bold text-pittsburgh-black mb-6">Weekly Calendar</h2>
                  <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                    {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => (
                      <div key={day} className="bg-white rounded-lg shadow-sm p-4">
                        <h3 className="font-bold text-pittsburgh-black mb-2">{day}</h3>
                        {dealsByDay[day] ? (
                          <div className="space-y-2">
                            {dealsByDay[day].slice(0, 3).map(deal => (
                              <div key={deal.id} className="text-xs text-gray-600 p-2 bg-gray-50 rounded">
                                <div className="font-semibold truncate">{deal.business.name}</div>
                                <div className="text-pittsburgh-gold">{deal.discount?.display}</div>
                              </div>
                            ))}
                            {dealsByDay[day].length > 3 && (
                              <div className="text-xs text-gray-500">+{dealsByDay[day].length - 3} more</div>
                            )}
                          </div>
                        ) : (
                          <div className="text-xs text-gray-400">No specials</div>
                        )}
                      </div>
                    ))}
                  </div>
