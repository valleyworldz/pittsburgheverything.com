...
import { MapPin, Clock, Heart, Share2, Globe, Award, Dog } from 'lucide-react'
...
export default function DogParkCard({ park, onFavorite, isFavorite = false }: DogParkCardProps) {
      ...
      <div className="p-4">
        ...
        {park.features && park.features.length > 0 && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              {park.features.slice(0, 4).map((feature, idx) => (
                <span
                  key={idx}
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
            {park.amenities.fenced && <div>✓ Fenced</div>}
            {park.amenities.separateSmallDogArea && <div>✓ Small Dog Area</div>}
            {park.amenities.waterFountains && <div>✓ Water Fountains</div>}
            {park.amenities.wasteBags && <div>✓ Waste Bags</div>}
            {park.amenities.benches && <div>✓ Benches</div>}
            {park.amenities.parking && <div>✓ Parking</div>}
            {park.amenities.lighting && <div>✓ Lighting</div>}
            {park.amenities.agilityEquipment && <div>✓ Agility Equipment</div>}
          </div>
        </div>
        ...
        {park.rules && park.rules.length > 0 && (
          <div className="mb-3 p-2 bg-yellow-50 rounded">
            <h4 className="text-xs font-semibold text-gray-900 mb-1">Rules</h4>
            <ul className="text-xs text-gray-600 space-y-1">
              {park.rules.slice(0, 2).map((rule, idx) => (
                <li key={idx}>• {rule}</li>
              ))}
            </ul>
          </div>
